...
const LEGACY_SIGNUP_STORAGE_KEY = 'wrdlnkdn-signup-state';
...
const loadState = (): JoinState => {
  try {
    const saved =
      localStorage.getItem(JOIN_STORAGE_KEY) ??
      localStorage.getItem(LEGACY_SIGNUP_STORAGE_KEY);
    if (saved) {
      console.log('📦 Loading join state from localStorage:', saved);
      return JSON.parse(saved);
    }
  } catch (e) {
    console.error('Failed to load join state:', e);
  }
  return initialState;
};
...
export const JoinProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    try {
      ...
      console.error('Failed to save join state:', e);
    }
  }, [state]);
