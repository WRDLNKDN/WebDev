...
test.describe('Portfolio artifact editing', () => {
  test('edits an existing artifact from dashboard and reflects changes on profile', async ({
    ...
    const editLegacyButton = page.getByLabel('Edit project Legacy Artifact');
    await expect(editLegacyButton).toBeVisible({ timeout: 30_000 });
    await editLegacyButton.click();
