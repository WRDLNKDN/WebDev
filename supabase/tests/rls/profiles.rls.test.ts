...
const SUPABASE_ANON_KEY = requireEnv("SUPABASE_ANON_KEY", process.env.SUPABASE_ANON_KEY);
...
async function createUserViaAdmin(admin: SupabaseClient, email: string, password: string) {
  ...
  if (!data.user?.id) throw new Error("Failed to create test user (no id returned)");
...
async function signInAsUser(email: string, password: string) {
  ...
  const { data: login, error: loginErr } = await loginClient.auth.signInWithPassword({
    email,
    password,
  });
...
describe("RLS: public.profiles", () => {
  ...
  beforeAll(async () => {
    ...
    const { error: insertErr } = await authedClient.from("profiles").insert({
      ...
      socials: { discord: "nick#0001", reddit: null, github: "NickTheDevOpsGuy" },
    });
