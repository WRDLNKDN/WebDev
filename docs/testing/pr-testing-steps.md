...
- [ ] **Click a match**  
      Navigate to that person’s profile at `/profile/{handle}`; dropdown closes.
...
- [ ] **Search button**  
      Type a query and click “Search” (or Enter) → go to `/directory?q=...` with
      the query.
...
- [ ] **Right column**  
      “WRDLNKDN News” (two items). “Your Connections” with search and connection
      rows (avatar, name, link).

- [ ] **Posting**  
      Type in “Start a post”, click Post → post appears (if API is up). Sort and
      feed load behave as before.
